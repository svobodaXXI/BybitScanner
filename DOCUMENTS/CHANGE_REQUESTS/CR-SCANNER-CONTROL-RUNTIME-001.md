...
```json
{
  ...
  "risks": [
    ...
    "SCHEMA VERSION BRANCH COLLISION (found and fixed before commit): this CR's scanner_runtime_state migration was initially numbered SCHEMA_VERSION 16, but the separate robot-v0-1-admission-gate branch independently claimed v16 the same day for its own migration (adding PAUSED to robot_runtime_state) -- both branches diverged from the same v15 base. Renumbered this CR's migration to v17 before commit to avoid a silent no-op collision (a database already migrated by one branch would show user_version=16 or 17 and the other branch's code would wrongly conclude nothing needs migrating). v16 (robot-v0-1-admission-gate) and v17 (main, this CR) are now both live on their respective branches and MUST be manually reconciled -- re-numbered into one consistent sequence, migration order verified, both branches' schema tests re-run -- whenever main and robot-v0-1-admission-gate are merged. This is not resolved by this revision, only avoided for now by using a number the other branch had not yet taken"
  ],
  "rollback_boundaries": [
