...
```json
{
  ...
  "related_work": [
    {
      "relationship": "CROSS_REFERENCE_NOT_A_REVISION_OF_THIS_CR",
      "reason": "Live PAPER acceptance testing on branch robot-v0-1-pause-stop-lifecycle (separate from this CR's own robot-v0-1-admission-gate line) found that RobotBreakoutMonitor -- out of this CR's own context_scope_paths/approved_scope entirely -- never consulted robot_runtime_state at all, so an already-APPROVED pre-entry candidate kept submitting/advancing new entry orders even while the robot was durably PAUSED via the commands this CR implemented (observed directly: NEOUSDT/HAJIMIUSDT LIMIT orders appearing after a real pause_robot() call). Per explicit user authorization in chat (\"PAUSE semantics = variant B\"), this gap was closed directly in terminal/application/robot_breakout_monitor.py (two risk-increasing pre-entry submission call sites gated on the same authoritative (mode, recovery_status) fields robot_admission.admit_robot_candidate() already reads) and recorded as new Section 7 of DOCUMENTS/AUTOPILOT_ROBOT_V0_1_ROBOT_CONTROL_DECISION.md (v1.2 -> v1.3). This CR's own approved_scope, context_scope_paths, and the six command functions themselves (Sections 1-6 of that decision) are unchanged by that work.",
      "date": "2026-09-14"
    },
    {
      "relationship": "OVERLAPS_THIS_CR_SCOPE_BY_EXPLICIT_USER_AUTHORIZATION",
      "reason": "A same-day safety review of the Section 7 (v1.3) work above found two further confirmed blockers -- pause_robot()/stop_robot() closed durable admission immediately but only cancelled a pending candidate's working entry LIMIT on RobotBreakoutMonitor's own periodic tick (a real fill-after-PAUSE/STOP race), and a partial fill blocked from completing could be left unprotected longer than the existing 5-second STOP-not-proven maximum. Per explicit user authorization in chat (the 'AUTHORITATIVE USER DECISIONS' closing message), pause_robot() and stop_robot() -- both originally implemented under THIS CR's approved_scope/context_scope_paths (terminal/application/robot_control.py) -- were changed to take a required http_post parameter and synchronously bridge into a new PaperRuntime.robot_synchronize_pending_entries() (reusing close_all_now()'s existing HTTP-bridge architecture from this CR's revision 1.2) before reporting success. This is the one point where the follow-on work directly modifies a function this CR delivered, rather than staying a pure cross-reference; recorded as AUTOPILOT_ROBOT_V0_1_ROBOT_CONTROL_DECISION.md v1.3 -> v1.4. All existing tests covering pause_robot()/stop_robot()'s legality matrix (tests/test_robot_control.py) were updated for the new required parameter, not reduced in coverage; see that file's own test count.",
      "date": "2026-09-14"
    }
  ]
}
