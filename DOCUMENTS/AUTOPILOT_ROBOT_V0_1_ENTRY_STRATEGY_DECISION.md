...
The threshold is `robot_protection.MIN_ENTRY_RR = 1.5`, overridable by the environment variable `ROBOT_MIN_ENTRY_RR` (a Decimal in 0..10; anything else falls back to 1.5). `MIN_LATE_ADMISSION_RR` of the late Market path shares the same default and is not affected by the variable. If the ratio cannot be computed, the LIMIT is placed as before and the existing post-fill protection path (with its fail-closed handling) stays the owner of that case. Only the initial LIMIT is checked; repricing of an unfilled LIMIT is unchanged. The late Market path keeps its own handling: `SKIPPED_POOR_RR` there does not terminate the candidate, it is re-evaluated on the next tick.
