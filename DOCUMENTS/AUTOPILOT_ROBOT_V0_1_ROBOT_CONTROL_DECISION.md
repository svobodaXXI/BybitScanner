...
**Amendment (v1.2 → v1.3):** Live PAPER acceptance testing surfaced a real gap this decision had never addressed: Sections 2/5 fix what `pause_robot()`/`stop_robot()` do to *new* candidate admission and to an *already-open* Robot position, but neither said anything about a candidate that was admitted (Scanner signal already approved into durable `APPROVED` state) *before* the pause/stop yet has not yet reached a fill — i.e. `RobotBreakoutMonitor`'s own in-flight pre-entry candidates (retest detection, initial entry LIMIT submission, partial-fill top-up). Because that monitor never consulted `robot_runtime_state` at all, an already-`APPROVED` pre-entry candidate kept submitting/advancing new entry orders even while the robot was `PAUSED` (observed directly: NEOUSDT/HAJIMIUSDT LIMIT orders appearing after a `pause_robot()` call). v1.3 adds Section 7 to close this gap, per the user's authoritative "PAUSE semantics = variant B" decision in chat. This is also the first section of this document with implementation authorization; Sections 1-6 remain design-only.

**Amendment (v1.3 → v1.4):** A subsequent safety review of the v1.3 implementation confirmed a real race: `pause_robot()`/`stop_robot()` closed durable admission immediately, but Section 7's cancellation/terminalization of a pending candidate's working entry LIMIT happened only on `RobotBreakoutMonitor`'s own periodic tick (`tick_interval_s`, 60s by default) — during that window a resting order could still fill and create new exposure after PAUSE/STOP were already asserted. The review separately confirmed that a partial fill left waiting on a blocked Market top-up could go unprotected indefinitely, longer than the 5-second STOP-not-proven maximum `AUTOPILOT_ROBOT_V0_1_RECOVERY_STATE_BATCH_DECISION.md` Section 6 already accepts. v1.4 closes both: `pause_robot()`/`stop_robot()` now synchronously bridge into Section 7's reconciliation (reusing the `close_all_now()` HTTP-bridge architecture) before reporting success, and a partial fill blocked from a Market top-up is now finalized/protected for its actual filled quantity immediately rather than left waiting. See Section 7's own amendment for the full mechanism; Sections 2 and 5 below are corrected in one place each (their "does not itself cancel a working entry/top-up order" language predates this fix).
...
Pause is a durable admission gate, not an account-flattening action. It does not itself request any close, cancel, or amend against an open position.

**Correction (v1.4):** the sentence above originally also claimed pause_robot() never cancels a working entry/top-up order. That is no longer accurate for a *pending pre-entry* candidate (one not yet `OPEN`): `pause_robot()` now synchronously reconciles those through Section 7 before reporting success, specifically to prevent a still-working entry LIMIT from filling into a new position after PAUSE was asserted. An already-*open* Robot position's STOP/TAKE/protection is still never touched by this — see Section 7 for the exact boundary.
...
**Addition (v1.4):** after committing `ROBOT_STOPPED`, `stop_robot()` now synchronously reconciles every pending (not yet `OPEN`) Robot candidate through Section 7 before reporting success: a still-working entry LIMIT is cancelled and a zero-exposure candidate is given its terminal `INVALIDATED` disposition immediately, instead of waiting for `RobotBreakoutMonitor`'s own periodic tick. This still never initiates a close of an *open* position — that remains exclusively `close_all_now()`'s responsibility, unchanged from v1.1 above.
...
## 7. Already-admitted pre-entry candidates under PAUSE/STOP/RECONCILIATION_REQUIRED (v1.3 — new; v1.4 — synchronous bridge + partial-fill protection; v1.5 — decoupled from closed-candle availability; IMPLEMENTED)

Scope: this section governs a durable `robot_candidates` row that is already `APPROVED` (Scanner signal already admitted) but has not yet reached `OPEN` (no `robot_trade` created yet) — i.e. exactly the candidates `RobotBreakoutMonitor` (the sole owner of this lifecycle segment) is still driving toward a fill. Sections 2 and 5 above already cover new-candidate admission and already-`OPEN` positions respectively; this section covers the gap between them.

**PAUSE (`recovery_status=PAUSED`) and RECONCILIATION_REQUIRED — block and cancel, stay recoverable:**

- No new entry order (initial retest LIMIT, or a partial fill's Market completion) may be submitted for such a candidate. It remains `APPROVED`, retried every subsequent tick, and resumes normal advancement automatically once admission returns to `(ROBOT_RUNNING, READY)` — no separate recovery action is required for it specifically.
- A still-working, not-yet-filled entry LIMIT already submitted before the pause/reconciliation is cancelled through the existing sanctioned `cancel_limit`/execution path — the same one already used elsewhere in this monitor for partial-fill remainder cancellation. No direct persistence manipulation fakes this cancellation.
- A candidate that already has a partial fill (real, non-zero exposure, no `robot_trade` yet) is never invalidated, and its resting remainder is still cancelled as usual, but completing it via a new Market order is withheld the same way. **Correction (v1.4):** it is not merely left `APPROVED` and blocked waiting for admission to be restored — see "Partial fill is exposure, not a pending candidate" below. The original v1.3 text understated this: a filled quantity, however partial, is exposure now and gets its own protection lifecycle immediately, never left waiting on a Market top-up that may not come while blocked.
- None of this touches an already-`OPEN` candidate (a real `robot_trade` already exists): `RobotBreakoutMonitor` already skips every non-`APPROVED` candidate entirely, so its STOP/TAKE/protection/recovery lifecycle (Section 2, Section 5) is structurally untouched by any of the above.

**ROBOT_STOPPED — terminal disposition for pending pre-entry intent, consistent with `AUTOPILOT_ROBOT_V0_1_RESTART_FROM_STOPPED_DECISION.md`:**

- Everything PAUSE does above (block submission, cancel a still-working entry LIMIT) also applies under `ROBOT_STOPPED`.
- In addition, if no fill/exposure exists yet for the candidate (no order ever submitted, or a resting order still at zero fill), it receives an explicit durable terminal disposition: the existing terminal `INVALIDATED` candidate status (already established by the CR-PAPER-PROTECTION-LIFECYCLE-001 fail-closed emergency-close path), reusing the existing `save_robot_candidate_state` mechanism — no new candidate status or transition is introduced.
- A candidate that already has a partial fill at the moment `ROBOT_STOPPED` is reached is deliberately **not** invalidated here (see the general partial-fill rule above): doing so would durably terminalize a candidate while real exposure remains open with no `robot_trade`/protection ownership record pointing at it. It is finalized and protected for its actual filled quantity instead (see "Partial fill is exposure, not a pending candidate" below), exactly like the PAUSE/RECONCILIATION_REQUIRED case.
- Once a candidate reaches `INVALIDATED` here, it is permanently terminal: a later `start_robot()`/restart never revives it (the existing `ROBOT_CANDIDATE_TRANSITIONS` table only allows `INVALIDATED -> INVALIDATED`), and `RobotBreakoutMonitor.tick()`'s own `status == "APPROVED"` filter permanently excludes it. New Robot work after a restart only ever originates from a genuinely new candidate admission (Section 1), never from reviving this one.

**Never gated by any of the above:** an already-filled entry's protection establishment (`_finalize_trade`, `submit_initial_protection`) and the existing fail-closed emergency-close path (`_fail_closed_unprotected_fill`, CR-PAPER-PROTECTION-LIFECYCLE-001) remain fully unconditional on admission state. A fill that already exists is never left unprotected because the robot happens to be paused, stopped, or reconciling — Section 7 governs only whether a *new* entry-risk order may be submitted, never whether an existing fill gets protected or, on failure, emergency-closed.

**Partial fill is exposure, not a pending candidate (v1.4 — new; v1.5 — decoupled from closed-candle availability; IMPLEMENTED):** a safety review of the v1.3 implementation found that leaving a partial fill merely "blocked" while waiting for admission to be restored could leave real, non-flat, robot-owned exposure with no proven STOP for longer than the 5-second maximum `AUTOPILOT_ROBOT_V0_1_RECOVERY_STATE_BATCH_DECISION.md` Section 6 already accepts (that document's "filled entry" / "open robot-owned position" language does not exempt a fill merely because it is smaller than originally intended). v1.4 treats any authoritatively filled quantity, however partial, as exposure requiring its own protection lifecycle now: when the admission gate would withhold a partial fill's Market top-up, the actual filled quantity is finalized and protected immediately instead — reusing the exact same `_finalize_trade()`/`build_protection_plan()`/`submit_initial_protection()`/`_fail_closed_unprotected_fill()` machinery every other entry completion already uses, sized to the real, authoritatively-observed filled fraction (never the originally-intended full wave-volume, never a synthesized quantity — `entry_quantity`/`entry_position_version` are still re-derived from the authoritative position projection exactly as before). This candidate is still never invalidated for being blocked, under any of PAUSE/STOP/RECONCILIATION_REQUIRED; only `_finalize_trade()`'s own existing fail-closed path can terminalize it from here, exactly as for a full fill. **v1.5 correction:** the v1.4 implementation placed this immediate-finalize call after the surrounding partial-fill-decision logic's `get_closed_candle()` gate, so a missing closed candle at the exact moment this ran could still defer protection to `RobotBreakoutMonitor`'s own periodic tick -- reintroducing the 60-second dependency this section exists to remove. v1.5 moves the blocked-admission check ahead of that gate: every input the immediate finalize needs is already frozen/durable or comes from the authoritative position projection, so no closed candle is read on this path at all.
