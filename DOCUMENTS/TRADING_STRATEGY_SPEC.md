...
```text
initial_risk_distance_after_costs = entry_price - selected_stop_price + expected_entry_exit_cost_distance
expected_reward_distance_after_costs = upper_wedge_boundary_price_at_decision - entry_price - expected_entry_exit_cost_distance
...
For this pre-breakout LONG, the primary `expected_reward` reference is the contemporaneous **upper boundary of the
same Falling Wedge** as known at the decision/entry time. It is the nearest structural objective available without
assuming that a breakout will happen. The boundary must be evaluated from the versioned geometry available at that
time; later boundary revisions may not rewrite the original admission calculation.

Potential beyond the upper boundary is deliberately excluded from the pre-breakout admission numerator. A projected
post-breakout target, measured-move potential, runner extension or later trend continuation may affect trade
management only after the corresponding breakout evidence exists; none may be used beforehand to make an otherwise
sub-`1:1` pre-breakout entry appear admissible. If the upper-boundary reward after expected costs is smaller than the
selected initial risk, the candidate entry is rejected.

The `1.0` minimum is itself `NEEDS VALIDATION`; a `2%` stop does not by itself guarantee `1:1`. Fees, spread,
slippage and expected execution costs must be included consistently on both sides of the comparison. Alternative
reward references may be studied only as separately versioned controls and must not silently replace the upper-edge
baseline after outcomes are known.
...
* compare `STRUCTURAL_EXTREMUM_STOP`, `CONFIRMATION_CANDLE_OPEN_STOP` and the candidate distance-based selector;
  ...
  target as the frozen baseline, including stop-out-then-MFE analysis and volatility/noise sensitivity;
...
13. For Falling Wedge lower-edge pre-breakout entry, does the candidate selector
    ...
    `expected_reward_to_risk >= 1.0`, calculated to the contemporaneous upper wedge boundary after costs, improve
    outcomes without excluding too many valid entries? Are both thresholds robust across neighboring values and
    volatility regimes?
