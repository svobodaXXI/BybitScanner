...
В ChatGPT используются два внешних слоя:

- **Global Custom Instructions** — короткий условный bootstrap, потому что они
  применяются ко всем чатам;
- **BybitScanner Project Instructions** — полный проектный контракт только
  внутри проекта; внутри проекта он имеет приоритет над глобальными
  инструкциями.

Оба канонических блока и правила освежения контекста хранятся в
`DOCUMENTS/CUSTOM_INSTRUCTIONS_BYBITSCANNER.md`. Ни один из UI-слоёв не хранит
текущий квест/XP/SHA/PR/runtime state — это остаётся в репозитории.

В уже идущем чате применяется быстрое освежение, если владелец прямо просит
освежить контекст, возвращается к BybitScanner после заметного перерыва,
репозиторное состояние могло измениться или обнаружено противоречие между
памятью чата и GitHub authority. Такое освежение не должно превращаться в
полный Project Sync без отдельной причины.
