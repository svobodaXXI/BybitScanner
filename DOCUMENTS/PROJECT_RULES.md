...
При структурном изменении документа
ассистент должен предоставлять
полный актуальный документ.

Частичные ручные изменения документа
не являются предпочтительным способом
сопровождения канонической документации.

---

## 3.4 Technical Artifact Delivery

Перед выдачей технического документа
должна быть предоставлена команда
для его открытия или создания.
...
## 4.1 Complete Module Delivery

При структурном изменении программного модуля
модуль должен предоставляться целиком
как готовый файл.
...
Document Update допускается только
при наличии валидного migration context
...
Перед структурным изменением необходимо
учитывать:
...
6. состояние документации.

Изменение не должно выполняться
только на основании локального
...
## 17.2 Complete Artifacts

Канонические документы и программные
модули при структурных изменениях
...
## 20.10 Notepad Delivery

Если необходимо получить содержимое
файла от пользователя, ассистент должен
выдать точную команду:

```powershell
notepad C:\BybitScanner\path\to\file
```

Получение содержимого через длинный
`Get-Content` в PowerShell не является
предпочтительным способом.

`Get-Content` не следует использовать
для выдачи больших программных модулей
или технических документов, если
`notepad` позволяет получить содержимое
удобнее.

---

## 20.11 Context Window Monitoring

Ассистент должен контролировать
примерный остаток доступного контекстного
окна в процессе длительной рабочей сессии.

В рабочем процессе должен периодически
выдаваться показатель:

`Context Remaining: приблизительно XX%`

Показатель:

* является приблизительной оценкой;
* не является точным системным значением;
* не должен представляться как измерение
  внутреннего технического счётчика;
* используется для предупреждения пользователя
  о приближении к необходимости компактной
  фиксации состояния проекта.

При существенном уменьшении остатка контекста
ассистент должен учитывать риск потери рабочего
контекста и при необходимости приоритизировать:

* сохранение текущего состояния;
* актуализацию Project State;
* фиксацию незавершённого этапа;
* сохранение необходимых артефактов;
* предотвращение повторной выдачи уже завершённых
  артефактов.

Показатель Context Remaining не должен
нарушать Artifact First Policy и не должен
заменять требуемый технический результат.
